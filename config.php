...
$isforkable=false;
if (file_exists('./d/isforkable.txt')){
	$isforkable=boolval('./d/isforkable.txt');
	}


$homepage_content='';
if (file_exists('homepagecontent.txt')) 
	{
			$homepage_content=file_get_contents('homepagecontent.txt');
		}
$mediatiers=Array();
if (file_exists('./d/RadioMediaTiers.txt')){
	...
	$soctokens=explode("\n", $socdata);
		...
		while ($p<count($soctokens)){
			...
			$p++;
			$socialone['download']=boolval($soctokens[$p]);
			$p++;
			$socialone['weight']=intval($soctokens[$p]);
			$p++;
			$socialone['whitelist']=boolval($soctokens[$p]);
			$p++;
			$socialone['audio_url']=$soctokens[$p];
